...
circle_kernel(NULL), r2_kernel(NULL), ramp_x_kernel(NULL), ramp_y_kernel(NULL)
{
	...
	status = ippiAdd_16s_C1RSfs(ramp_x, ramp_step, ramp_y, ramp_step, r2, ramp_step, ramp_roi_size,0);

	status = ippiCopy_16s_C1R(r2, ramp_step, circle, ramp_step, ramp_roi_size);

	//trim away values greater than radius^2 for r2 kernel
	status = ippiThreshold_GTVal_16s_C1IR(r2, ramp_step, ramp_roi_size, radius * radius, 0);
	...
	constantoffset = 1;
	status = ippiImageRamp_16s_C1R(ramp_x, ramp_step, ramp_roi_size, constantoffset, 1, ippAxsHorizontal);
	status = ippiImageRamp_16s_C1R(ramp_y, ramp_step, ramp_roi_size, constantoffset, 1, ippAxsVertical);

	//multiply ramps by circle kernel to eliminate all values beyond the radius
	status = ippiMul_16s_C1IRSfs(circle, ramp_step, ramp_x, ramp_step, ramp_roi_size, 0);
	status = ippiMul_16s_C1IRSfs(circle, ramp_step, ramp_y, ramp_step, ramp_roi_size, 0);

	//Convert kernels to final forms
	status = ippiConvert_16s32f_C1R(circle, ramp_step, circle_kernel, convolution_kernel_step, ramp_roi_size);
	status = ippiConvert_16s32f_C1R(r2, ramp_step, r2_kernel, convolution_kernel_step, ramp_roi_size);
	status = ippiConvert_16s32f_C1R(ramp_x, ramp_step, ramp_x_kernel, convolution_kernel_step, ramp_roi_size);
	status = ippiConvert_16s32f_C1R(ramp_y, ramp_step, ramp_y_kernel, convolution_kernel_step, ramp_roi_size);
